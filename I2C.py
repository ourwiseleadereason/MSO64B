...
rootPath = os.getcwd()
csvPath = os.path.join(rootPath, 'test data', 'csv')
...
for idn in id_list:
    ...
    if int(num_csv) == 7:
        ...
        if type(sheet.cell(id_row, 16).value) is int or float:
            ...
            id_row, 16).value
            sheet.cell(id_row + 1, 19).value = (sheet.cell(id_row + 1, 18).value -
                                                sheet.cell(id_row, 16).value) / sheet.cell(id_row, 16).value
